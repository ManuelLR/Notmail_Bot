...
class UserContent:
    ...
    def add_email(self, smtp_server, smtp_server_port, from_email, from_pwd, refresh_time=None):
        if refresh_time == None or not isinstance(refresh_time, int) or refresh_time < 1:
            refresh_time = get_config().default_refresh_inbox
        self.messages[from_email] = MessageContent(smtp_server, smtp_server_port, from_email, from_pwd, refresh_time=refresh_time)


class MessageContent:
    def __init__(self, smtp_server, smtp_server_port, from_email, from_pwd, refresh_time=None):
        if refresh_time == None or not isinstance(refresh_time, int) or refresh_time < 1:
            refresh_time = get_config().default_refresh_inbox
